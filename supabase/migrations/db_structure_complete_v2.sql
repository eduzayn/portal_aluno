-- Script completo para adicionar componentes faltantes na estrutura do banco de dados do Portal do Aluno
...
DROP TRIGGER IF EXISTS after_enrollment_update_for_learning_path ON enrollments;
...
DROP TRIGGER IF EXISTS update_learning_path_courses_updated_at ON learning_path_courses;
...
-- Funções para gerenciar configurações do portal
-- Nota: Adapte estas funções conforme a estrutura real da tabela portal_settings
...
DECLARE
  v_value TEXT;
BEGIN
  -- Adapte esta consulta conforme a estrutura real da tabela portal_settings
...
BEGIN
  ...
  IF NOT FOUND THEN
    INSERT INTO portal_settings (setting_key, setting_value, description, setting_type)
    VALUES (p_key, p_value, p_description, p_type);
  END IF;
...
DROP POLICY IF EXISTS learning_paths_select_policy ON learning_paths;
...
DROP POLICY IF EXISTS learning_paths_insert_policy ON learning_paths;
...
DROP POLICY IF EXISTS learning_paths_update_policy ON learning_paths;
...
DROP POLICY IF EXISTS learning_paths_delete_policy ON learning_paths;
...
DROP POLICY IF EXISTS learning_path_courses_select_policy ON learning_path_courses;
...
DROP POLICY IF EXISTS learning_path_courses_insert_policy ON learning_path_courses;
CREATE POLICY learning_path_courses_insert_policy ON learning_path_courses
FOR INSERT WITH CHECK (auth.role() = 'authenticated' AND auth.jwt() ->> 'role' IN ('admin', 'instructor'));

DROP POLICY IF EXISTS learning_path_courses_update_policy ON learning_path_courses;
CREATE POLICY learning_path_courses_update_policy ON learning_path_courses
FOR UPDATE USING (auth.role() = 'authenticated' AND auth.jwt() ->> 'role' IN ('admin', 'instructor'));

DROP POLICY IF EXISTS learning_path_courses_delete_policy ON learning_path_courses;
...
DROP POLICY IF EXISTS learning_path_enrollments_select_policy ON learning_path_enrollments;
...
DROP POLICY IF EXISTS learning_path_enrollments_insert_policy ON learning_path_enrollments;
...
DROP POLICY IF EXISTS learning_path_enrollments_update_policy ON learning_path_enrollments;
...
DROP POLICY IF EXISTS learning_path_enrollments_delete_policy ON learning_path_enrollments;
...
DROP POLICY IF EXISTS supplementary_materials_select_policy ON supplementary_materials;
...
DROP POLICY IF EXISTS supplementary_materials_insert_policy ON supplementary_materials;
CREATE POLICY supplementary_materials_insert_policy ON supplementary_materials
FOR INSERT WITH CHECK (auth.role() = 'authenticated' AND auth.jwt() ->> 'role' IN ('admin', 'instructor'));

DROP POLICY IF EXISTS supplementary_materials_update_policy ON supplementary_materials;
CREATE POLICY supplementary_materials_update_policy ON supplementary_materials
FOR UPDATE USING (auth.role() = 'authenticated' AND auth.jwt() ->> 'role' IN ('admin', 'instructor'));

DROP POLICY IF EXISTS supplementary_materials_delete_policy ON supplementary_materials;
CREATE POLICY supplementary_materials_delete_policy ON supplementary_materials
FOR DELETE USING (auth.role() = 'authenticated' AND auth.jwt() ->> 'role' IN ('admin', 'instructor'));

-- Políticas para course_ratings
DROP POLICY IF EXISTS course_ratings_select_policy ON course_ratings;
...
DROP POLICY IF EXISTS course_ratings_insert_policy ON course_ratings;
...
DROP POLICY IF EXISTS course_ratings_update_policy ON course_ratings;
...
DROP POLICY IF EXISTS course_ratings_delete_policy ON course_ratings;
...
-- Nota: Adapte esta inserção conforme a estrutura real da tabela portal_settings
DO $$
BEGIN
  -- Verificar se a função update_portal_setting existe
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_portal_setting') THEN
    -- Usar a função para inserir configurações
    PERFORM update_portal_setting('site_name', 'Portal do Aluno - Edunéxia', 'Nome do site', 'text');
    PERFORM update_portal_setting('site_description', 'Portal do Aluno da Edunéxia - Ambiente digital completo para gerenciamento acadêmico', 'Descrição do site', 'text');
    PERFORM update_portal_setting('primary_color', '#4F46E5', 'Cor primária do tema', 'color');
    PERFORM update_portal_setting('secondary_color', '#10B981', 'Cor secundária do tema', 'color');
    PERFORM update_portal_setting('logo_url', '/images/logo.png', 'URL do logotipo', 'url');
    PERFORM update_portal_setting('favicon_url', '/favicon.ico', 'URL do favicon', 'url');
    PERFORM update_portal_setting('max_file_upload_size', '10485760', 'Tamanho máximo de upload de arquivos em bytes (10MB)', 'number');
    PERFORM update_portal_setting('allowed_file_types', 'pdf,doc,docx,ppt,pptx,xls,xlsx,jpg,jpeg,png,gif,mp4,mp3,zip', 'Tipos de arquivos permitidos para upload', 'text');
    PERFORM update_portal_setting('enable_notifications', 'true', 'Habilitar notificações', 'boolean');
    PERFORM update_portal_setting('enable_chat', 'true', 'Habilitar chat', 'boolean');
    PERFORM update_portal_setting('enable_forum', 'true', 'Habilitar fórum', 'boolean');
    PERFORM update_portal_setting('enable_calendar', 'true', 'Habilitar calendário', 'boolean');
    PERFORM update_portal_setting('enable_certificates', 'true', 'Habilitar certificados', 'boolean');
    PERFORM update_portal_setting('enable_financial', 'true', 'Habilitar módulo financeiro', 'boolean');
    PERFORM update_portal_setting('enable_learning_paths', 'true', 'Habilitar rotas de aprendizagem', 'boolean');
    PERFORM update_portal_setting('enable_course_ratings', 'true', 'Habilitar avaliações de cursos', 'boolean');
  END IF;
END $$;

-- Criar buckets de armazenamento faltantes
-- Nota: Isso não pode ser feito via SQL, deve ser feito via API ou Console do Supabase
-- Execute o script create-supabase-storage-buckets.js para criar os buckets
